...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "srv-mobile-repository-ear.ear");

        // Current directory is the root of the "srv-mobile-repository-web" project. Go up one level
        // and enter the "srv-mobile-repository-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../srv-mobile-repository-ejb/target/srv-mobile-repository-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "srv-mobile-repository-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/srv-mobile-repository-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "srv-mobile-repository-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
